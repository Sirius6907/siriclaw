...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/theonlyhennygod/siriusclaw/security/advisories/new)
...
SiriClaw implements defense-in-depth security:
...
SiriClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t siriusclaw .
docker inspect --format='{{.Config.User}}' siriusclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace sirius gateway
```
