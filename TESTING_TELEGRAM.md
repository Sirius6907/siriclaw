...
This guide covers testing the Telegram channel integration for SiriClaw.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug sirius channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: sirius onboard --channels-only
```
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time sirius channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep siriusclaw`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/siriusclaw` |
...
```bash
# Maximum logging
RUST_LOG=trace sirius channel start
...
# Validate config
cat ~/.siriusclaw/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [SiriClaw Main README](README.md)
- [Contributing Guide](CONTRIBUTING.md)
- [Issue Tracker](https://github.com/theonlyhennygod/siriusclaw/issues)
