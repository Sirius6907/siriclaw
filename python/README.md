...
Python companion package for [SiriClaw](https://github.com/siriclaw-labs/siriusclaw) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install siriusclaw-tools[discord]
```
...
# Run the CLI
siriusclaw-tools "List files in the current directory"
...
```python
from siriusclaw_tools import tool
...
| Feature | Rust SiriClaw | siriusclaw-tools |
|---------|---------------|----------------|
...
Use **Rust SiriClaw** for production edge deployments. Use **siriusclaw-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
