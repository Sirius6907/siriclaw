...
A complete toolkit for building AI-powered robots with SiriClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build SiriClaw with robot tools

```bash
# Clone and build
git clone https://github.com/siriclaw-labs/siriusclaw
cd siriusclaw
cargo build -p siriusclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.siriusclaw
cp crates/robot-kit/robot.toml ~/.siriusclaw/
cp crates/robot-kit/SOUL.md ~/.siriusclaw/workspace/
...
# Test in mock mode
./target/release/sirius agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/sirius agent -m "Move forward 1 meter"
```
...
```rust
use siriusclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `siriusclaw`, add a thin adapter that maps this
...
mkdir siriusclaw-robot-kit
cp -r target/release/siriusclaw siriusclaw-robot-kit/
cp -r examples/robot_kit siriusclaw-robot-kit/
cp -r ~/.siriusclaw siriusclaw-robot-kit/dot-siriusclaw

# Include models
mkdir -p siriusclaw-robot-kit/models
cp ~/.siriusclaw/models/ggml-base.bin siriusclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf siriusclaw-robot-kit.tar.gz siriusclaw-robot-kit/

# Copy to USB
cp siriusclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
