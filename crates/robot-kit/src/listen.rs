...
impl ListenTool {
    pub fn new(config: RobotConfig) -> Self {
        let recordings_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".siriusclaw/recordings"))
            .unwrap_or_else(|| PathBuf::from("/tmp/siriusclaw_recordings"));
    ...
    async fn transcribe(&self, audio_path: &Path) -> Result<String> {
        ...
        // whisper.cpp model path (typically in ~/.siriusclaw/models/)
        let model_path = directories::UserDirs::new()
            .map(|d| {
                d.home_dir()
                    .join(format!(".siriusclaw/models/ggml-{}.bin", model))
            })
            .unwrap_or_else(|| {
