...
impl SpeakTool {
    pub fn new(config: RobotConfig) -> Self {
        let audio_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".siriusclaw/tts_cache"))
            .unwrap_or_else(|| PathBuf::from("/tmp/siriusclaw_tts"));
    ...
    async fn speak(&self, text: &str, emotion: &str) -> Result<()> {
        ...
        let model_path = directories::UserDirs::new()
            .map(|d| {
                d.home_dir()
                    .join(format!(".siriusclaw/models/piper/{}.onnx", voice))
            })
    ...
    async fn play_sound(&self, sound: &str) -> Result<()> {
        let sounds_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".siriusclaw/sounds"))
            .unwrap_or_else(|| PathBuf::from("/usr/local/share/siriusclaw/sounds"));
