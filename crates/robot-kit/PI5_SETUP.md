...
Complete guide to setting up a SiriClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.siriusclaw/models
cp models/ggml-base.bin ~/.siriusclaw/models/
```
...
mkdir -p ~/.siriusclaw/models/piper
cd ~/.siriusclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.siriusclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build SiriClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/siriusclaw
cd siriusclaw

# Build robot kit
cargo build --release -p siriusclaw-robot-kit

# Build main siriusclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.siriusclaw
nano ~/.siriusclaw/robot.toml
```

```toml
# ~/.siriusclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.siriusclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/siriusclaw_sensors.fifo"
...
### Start SiriClaw Agent

```bash
# Configure SiriClaw to use robot tools
cat > ~/.siriusclaw/config.toml << 'EOF'
...
cp ~/siriusclaw/crates/robot-kit/SOUL.md ~/.siriusclaw/workspace/

# Start agent
./target/release/sirius agent
```
...
# Start sensor loop
if [ ! -p /tmp/siriusclaw_sensors.fifo ]; then
    mkfifo /tmp/siriusclaw_sensors.fifo
fi
...
```bash
# /etc/systemd/system/siriusclaw-robot.service
sudo tee /etc/systemd/system/siriusclaw-robot.service << 'EOF'
[Unit]
Description=SiriClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/siriusclaw
...
sudo systemctl enable siriusclaw-robot
sudo systemctl start siriusclaw-robot

# Check status
sudo systemctl status siriusclaw-robot
journalctl -u siriusclaw-robot -f  # View logs
```
