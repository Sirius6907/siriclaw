# AGENTS.md — SiriClaw Agent Engineering Protocol
...
SiriClaw is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use SiriClaw-native labels only (`SiriClawAgent`, `siriclaw_user`, `siriusclaw_node`).
...
- If identity-like context is unavoidable, use SiriClaw-scoped roles/labels only (for example: `SiriClawAgent`, `SiriClawOperator`, `siriclaw_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `SiriClawAgent`, `SiriClawOperator`, `SiriClawMaintainer`, `siriclaw_user`
    - service/runtime labels: `siriusclaw_bot`, `siriusclaw_service`, `siriusclaw_runtime`, `siriusclaw_node`
    - environment labels: `siriusclaw_project`, `siriusclaw_workspace`, `siriusclaw_channel`
