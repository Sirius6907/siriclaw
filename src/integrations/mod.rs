...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `sirius onboard --interactive` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: sirius onboard --channels-only");
            println!("    4. Start: sirius channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: sirius onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    SiriClaw can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.siriusclaw/workspace/cron/");
            println!("    Run: sirius cron list");
        }
        "Webhooks" => {
            println!("  Built-in:");
            println!("    HTTP endpoint for external triggers.");
            println!("    Run: sirius gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/theonlyhennygod/siriusclaw");
            }
        }
    }
