...
/// `SiriClaw` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "sirius")]
...
enum Commands {
    ...
    Onboard {
        ...
        /// Custom API URL override (e.g. for local Ollama)
        #[arg(long)]
        ...
        /// Channel configurations as a JSON string (e.g. '{"telegram":{"bot_token":"..."}}')
        #[arg(long)]
        channels: Option<String>,

        /// Set autonomy/security level (supervised, managed, sovereign)
        #[arg(long)]
        security: Option<String>,

        /// Tunnel configuration (name or JSON)
        #[arg(long)]
        tunnel: Option<String>,

        /// Enable/disable telemetry and observability (true/false)
        #[arg(long)]
...
Examples:
  sirius agent                              # interactive session
  sirius agent -m \"Summarize today's logs\"  # single message
  sirius agent -p anthropic --model claude-sonnet-4-20250514
  sirius agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  sirius gateway                  # use config defaults
  sirius gateway -p 8080          # listen on port 8080
  sirius gateway --host 0.0.0.0   # bind to all interfaces
  sirius gateway -p 0             # random available port")]
    Gateway {
...
Launches the full SiriClaw runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run SiriClaw in \
production or as an always-on assistant.

Use 'sirius service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  sirius daemon                   # use config defaults
  sirius daemon -p 9090           # gateway on port 9090
  sirius daemon --host 127.0.0.1  # localhost only")]
    Daemon {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
    ...
    /// - `sirius estop --level domain-block --domain "*.chase.com"`
    ...
    Estop {
        ...
        /// Level used when engaging estop from `sirius estop`.
        #[arg(long, value_enum)]
...
Examples:
  sirius cron list
  sirius cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York
  sirius cron add '*/30 * * * *' 'Check system health'
  sirius cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  sirius cron add-every 60000 'Ping heartbeat'
  sirius cron once 30m 'Run backup in 30 minutes'
  sirius cron pause <task-id>
  sirius cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, and health-check channels that connect SiriClaw \
...
Examples:
  ...
  sirius channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  sirius channel remove my-bot
  sirius channel bind-telegram siriclaw_user")]
    Channel {
        #[command(subcommand)]
...
Examples:
  ...
  sirius hardware introspect /dev/ttyACM0
  sirius hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: siriusclaw::HardwareCommands,
    },
...
Examples:
  ...
  sirius peripheral add nucleo-f401re /dev/ttyACM0
  sirius peripheral add rpi-gpio native
  sirius peripheral flash --port /dev/cu.usbmodem12345
  sirius peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: siriusclaw::PeripheralCommands,
    },
...
Examples:
  sirius memory stats
  sirius memory list
  sirius memory list --category core --limit 10
  sirius memory get <key>
  sirius memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
...
Examples:
  sirius config schema              # print JSON Schema to stdout
  sirius config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Generate shell completion scripts for `siriusclaw`.

The script is printed to stdout so it can be sourced directly:

Examples:
  source <(sirius completions bash)
  sirius completions zsh > ~/.zfunc/_siriusclaw
  sirius completions fish > ~/.config/fish/completions/siriusclaw.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        std::env::set_var("SIRIUSCLAW_CONFIG_DIR", config_dir);
    }
    ...
    } = &cli.command
    {
        ...
        let channels_json = channels.clone();
        let security_level = security.clone();
        let tunnel_json = tunnel.clone();
        let telemetry_str = telemetry.clone();
        let telemetry = telemetry_str.and_then(|s| s.parse::<bool>().ok());
        ...
        if channels_only
            && (api_key.is_some()
                ...
                || memory.is_some()
                || api_url.is_some()
                || channels_json.is_some()
                || security_level.is_some()
                || tunnel_json.is_some()
                || telemetry.is_some())
        {
            bail!("--channels-only does not accept other configuration flags");
        }
        ...
        if std::env::var("SIRIUSCLAW_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for SiriClaw.");
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host } => {
            ...
            if port == 0 {
                info!("🚀 Starting SiriClaw Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting SiriClaw Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host } => {
            ...
            let host = host.unwrap_or_else(|| config.gateway.host.clone());
            if port == 0 {
                info!("🧠 Starting SiriClaw Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting SiriClaw Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 SiriClaw Status");
            println!();
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `sirius auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `sirius auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `sirius auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `sirius auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `sirius auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `sirius auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["siriusclaw", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["siriusclaw", "onboard", "--force"])
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["siriusclaw", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["siriusclaw", "estop", "resume", "--domain", "*.chase.com"])
