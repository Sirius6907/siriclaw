...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 SiriClaw Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `sirius doctor` again.");
    }
...
pub async fn run_models(
    ...
    println!("🩺 SiriClaw Doctor — Model Catalog Probe");
...
pub fn run_traces(
    ...
    println!();
    println!("Use `sirius doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `sirius onboard` to set one up",
        ));
    }
...
fn check_daemon_state(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if !state_file.exists() {
        items.push(DiagItem::error(
            cat,
            format!(
                "state file not found: {} — run `sirius daemon` to start background workers (scheduler, heartbeats)",
                state_file.display()
            ),
        ));
        return;
...
fn check_environment(items: &mut Vec<DiagItem>) {
    ...
    // Shell
    let shell = std::env::var("SHELL").unwrap_or_else(|_| std::env::var("COMSPEC").unwrap_or_default());
    if shell.is_empty() {
        if cfg!(windows) {
            items.push(DiagItem::ok(cat, "shell: powershell (default for Windows)"));
        } else {
...
mod tests {
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(first
            ...
            .is_some_and(|name| name.starts_with(".siriusclaw_doctor_probe_")));
    }
