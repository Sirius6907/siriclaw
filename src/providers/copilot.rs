...
/// Tokens are cached to `~/.config/siriusclaw/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "siriusclaw")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("siriusclaw-copilot-{user}"))
            });
