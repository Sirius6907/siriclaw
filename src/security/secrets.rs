...
// in `~/.siriusclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.siriusclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(siriusclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: siriusclaw_dir.join(".secret_key"),
            enabled,
        }
    }
