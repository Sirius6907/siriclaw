...
const BANNER: &str = r"
    ...
    Autonomous AI Agent Framework — 100% Rust. 100% Agnostic.
...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to SiriClaw — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 9, "Channels (How You Talk to SiriClaw)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("SIRIUSCLAW_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to SiriClaw)");
...
/// Use `sirius onboard` or `sirius onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `sirius onboard --interactive` for the full wizard.
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    let config_dir = home.join(".siriusclaw");
...
async fn run_quick_setup_with_home(
    ...
    let (siriusclaw_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if let Some(level) = security_level {
        autonomy.level = match level.to_lowercase().as_str() {
            "sovereign" | "full" => AutonomyLevel::Full,
            "managed" | "supervised" | _ => AutonomyLevel::Supervised,
        };
    }

    // Parse channels JSON
    let channels_config = if let Some(json) = channels_json {
        serde_json::from_str::<ChannelsConfig>(json)
            .context("Failed to parse --channels JSON")?
    } else {
        ChannelsConfig::default()
    };

    // Parse tunnel JSON or name
    let tunnel_config = if let Some(json) = tunnel_json {
        if json.starts_with('{') {
            serde_json::from_str::<crate::config::TunnelConfig>(json)
                .context("Failed to parse --tunnel JSON")?
        } else {
            let mut t = crate::config::TunnelConfig::default();
            t.provider = json.to_string();
            t
        }
    } else {
    ...
    if let Some(enabled) = telemetry_override {
        observability.backend = if enabled { "open-telemetry".into() } else { "none".into() };
    }
    ...
    println!(
        "  {} Security:   {}",
        style("✓").green().bold(),
        style(format!("{:?} ({})", autonomy.level, if security_level.is_some() { "explicit" } else { "default" })).green()
    );
    println!(
        "  {} Channels:   {}",
        style("✓").green().bold(),
        if channels_json.is_some() {
            style("configured via CLI").green()
        } else {
            style("default (CLI only)").dim()
        }
    );
    println!(
        "  {} Memory:     {} (auto-save: {})",
    ...
    println!(
        "  {} Tunnel:     {}",
        style("✓").green().bold(),
        if tunnel_json.is_some() {
            style(format!("{:?}", tunnel_config.provider)).green()
        } else {
            style("none (local only)".to_string()).dim()
        }
    );
    println!(
        "  {} Telemetry:  {}",
        style("✓").green().bold(),
        style(&observability.backend).green()
    );
    println!();
    println!(
        "  {} {}",
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     sirius agent -m \"Hello!\"");
            println!("    2. Gateway:  sirius gateway");
            println!("    3. Status:   sirius status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             sirius auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              sirius agent -m \"Hello!\"");
                println!("    3. Gateway:           sirius gateway");
                println!("    4. Status:            sirius status");
            }
        } else {
            let env_var = provider_env_var(&provider_name);
            println!("    1. Set your API key:  export {env_var}=\"sk-...\"");
            println!("    2. Or edit:           ~/.siriusclaw/config.toml");
            println!("    3. Chat:              sirius agent -m \"Hello!\"");
            println!("    4. Gateway:           sirius gateway");
        }
    } else {
        println!("    1. Chat:     sirius agent -m \"Hello!\"");
        println!("    2. Gateway:  sirius gateway");
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `sirius models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: sirius models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("SiriClaw works with ANY API that speaks the OpenAI chat completions format.");
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("SiriClaw will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how SiriClaw connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("SiriClaw uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("SiriClaw can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("SiriClaw can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should SiriClaw interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how SiriClaw stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to SiriClaw from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to SiriClaw from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to SiriClaw from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to SiriClaw from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("SiriClaw reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.siriusclaw/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("siriusclaw-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via SIRIUSCLAW_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::Lark | ChannelMenuChoice::Feishu => {
                ...
                println!(
                    ...
                    style(format!("— talk to SiriClaw from {provider_label}")).dim()
                );
                print_bullet(&format!(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("SiriClaw will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("SiriClaw is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("sirius auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: sirius auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("sirius channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("sirius agent -m \"Hello, SiriClaw!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("sirius agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("sirius status").yellow());
...
mod tests {
    ...
    async fn quick_setup_model_override_persists_to_config_toml() {
        ...
        let _workspace_env = EnvVarGuard::unset("SIRIUSCLAW_WORKSPACE");
        let _config_env = EnvVarGuard::unset("SIRIUSCLAW_CONFIG_DIR");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        let _config_env = EnvVarGuard::unset("SIRIUSCLAW_CONFIG_DIR");
        let tmp = TempDir::new().unwrap();
        let siriusclaw_dir = tmp.path().join(".siriusclaw");
        let config_path = siriusclaw_dir.join("config.toml");

        tokio::fs::create_dir_all(&siriusclaw_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("siriusclaw-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".siriusclaw").join("config.toml");

        let _workspace_env = EnvVarGuard::set(
            "SIRIUSCLAW_WORKSPACE",
        ...
        .await
        .expect("quick setup should honor SIRIUSCLAW_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** SiriClaw"),
            "should default agent name to SiriClaw"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("SiriClaw-v2"));
    }
