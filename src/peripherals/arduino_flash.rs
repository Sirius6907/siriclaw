//! Flash SiriClaw Arduino firmware via arduino-cli.
//!
...
/// SiriClaw Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/siriusclaw-arduino/siriusclaw-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "siriusclaw-arduino";
...
/// Flash SiriClaw firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("siriusclaw_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling SiriClaw Arduino firmware...");
    ...
    println!("SiriClaw firmware flashed successfully.");
