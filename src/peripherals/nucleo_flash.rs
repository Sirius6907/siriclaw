//! Flash SiriClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash SiriClaw Nucleo firmware. Builds from firmware/siriusclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (siriusclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("siriusclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from siriusclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building SiriClaw Nucleo firmware...");
    ...
    println!("SiriClaw Nucleo firmware flashed successfully.");
