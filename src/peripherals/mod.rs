...
/// Handle `sirius peripheral` subcommands.
    ...
    match cmd {
        crate::PeripheralCommands::List => {
            ...
            if boards.is_empty() {
                ...
                println!();
                println!("Add one with: sirius peripheral add <board> <path>");
                println!("  Example: sirius peripheral add nucleo-f401re /dev/ttyACM0");
                println!();
