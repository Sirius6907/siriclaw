...
Examples:
  ...
  sirius channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  sirius channel bind-telegram 123456789")]
    BindTelegram {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `SiriClaw` workspace
    Openclaw {
...
Examples:
  ...
  sirius cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  sirius cron add-every 60000 'Ping heartbeat'     # every minute
  sirius cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  sirius cron once 2h 'Follow up on deployment'
  sirius cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  sirius cron update <task-id> --expression '0 8 * * *'
  sirius cron update <task-id> --tz Europe/London --name 'Morning check'
  sirius cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  sirius hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  sirius peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash SiriClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash SiriClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  sirius peripheral flash
  sirius peripheral flash --port /dev/cu.usbmodem12345
  sirius peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash SiriClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
