...
const SERVICE_LABEL: &str = "com.siriusclaw.daemon";
const WINDOWS_TASK_NAME: &str = "SiriClaw Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "siriusclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["siriusclaw", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "siriusclaw.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["siriusclaw", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "siriusclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["siriusclaw", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["siriusclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/siriusclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/siriusclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "siriusclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove siriusclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/siriusclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: sirius service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=SiriClaw daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "siriusclaw.service"]));
...
/// Check if the siriusclaw user exists and has expected properties.
...
fn check_siriclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "siriusclaw"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser siriusclaw && delgroup siriusclaw",
            "addgroup -S siriusclaw && adduser -S -s /sbin/nologin -H -D -G siriusclaw siriusclaw",
        )
    } else {
        ("userdel siriusclaw", "useradd -r -s /sbin/nologin siriusclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'siriusclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'siriusclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/siriusclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: siriusclaw user has home directory '{}' (expected /var/lib/siriusclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_siriclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "siriusclaw"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_siriclaw_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "siriusclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "siriusclaw"])
                .output()
                .context("Failed to create siriusclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create siriusclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: siriusclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create siriusclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create siriusclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "siriusclaw"])
            .output()
            .context("Failed to create siriusclaw user")?;
    ...
    println!("✅ Created system user: siriusclaw");
    Ok(())
}

/// Change ownership of a path to siriusclaw:siriusclaw
#[cfg(unix)]
fn chown_to_siriusclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["siriusclaw:siriusclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to siriusclaw:siriusclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_siriusclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_siriusclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "siriusclaw:siriusclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to siriusclaw:siriusclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_siriusclaw(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".siriusclaw"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'siriusclaw' cannot write {} ({details}). \
             Re-run `sudo sirius service install` and ensure ownership is siriusclaw:siriusclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/siriusclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="siriusclaw:siriusclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/siriusclaw/access.log"
error_log="/var/log/siriusclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/siriusclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo sirius service install"
        );
    }

    ensure_siriclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/siriusclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/siriusclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_siriusclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by siriusclaw:siriusclaw)",
    ...
    chown_recursive_to_siriusclaw(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/siriusclaw");
    ...
    run_checked(Command::new("rc-update").args(["add", "siriusclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/siriusclaw");
    println!("   Config path: /etc/siriusclaw/config.toml");
    println!("   Start with: sudo sirius service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("siriusclaw-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/siriusclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "SiriClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/siriusclaw");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/siriusclaw"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"siriusclaw\""));
        assert!(script.contains("description=\"SiriClaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/siriusclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/sirius daemon\""));
        assert!(!script.contains("env SIRIUSCLAW_CONFIG_DIR"));
        assert!(!script.contains("env SIRIUSCLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"siriusclaw:siriusclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/siriusclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/siriusclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/siriusclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/siriusclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/siriusclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/siriusclaw"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/siriusclaw'".to_string()
            ]
        );
    }
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/siriusclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/siriusclaw/workspace'".to_string(),
                "siriusclaw".to_string()
            ]
        );
    }
