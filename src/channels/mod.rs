...
//! SiriClaw to external platforms. Each channel implements the [`Channel`] trait
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "siriusclaw.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "siriusclaw.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["siriusclaw", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["siriusclaw", "restart"];
...
async fn load_runtime_defaults_from_config_file(path: &Path) -> Result<ChannelRuntimeDefaults> {
    ...
    if let Some(siriusclaw_dir) = path.parent() {
        let store = crate::security::SecretStore::new(siriusclaw_dir, parsed.secrets.encrypt);
...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `sirius models refresh --provider {}`.",
...
pub fn build_system_prompt_with_mode(
    ...
    if prompt.is_empty() {
        "You are SiriClaw, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct."
            .to_string()
    } else {
        prompt
    }
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `sirius onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `sirius daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `sirius service stop && sirius service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.siriusclaw.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.siriusclaw.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.siriusclaw.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.siriusclaw.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/siriusclaw");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: sirius channel start");
            println!("To check health:    sirius channel doctor");
            println!("To configure:      sirius onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `sirius onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.siriusclaw/config.toml directly");
        }
...
fn collect_configured_channels(
    ...
    if let Some(ref mx) = config.channels_config.matrix {
        channels.push(ConfiguredChannel {
            display_name: "Matrix",
            channel: Arc::new(MatrixChannel::new_with_session_hint_and_siriusclaw_dir(
                mx.homeserver.clone(),
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `sirius onboard` first.");
        return Ok(());
    }

    println!("🩺 SiriClaw Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `sirius gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let provider_runtime_options = providers::ProviderRuntimeOptions {
        ...
        siriusclaw_dir: config.config_path.parent().map(std::path::PathBuf::from),
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `sirius onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 SiriClaw Channel Server");
    println!("  🤖 Model:    {model}");
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(crate::config::ReliabilityConfig::default()),
            provider_runtime_options: providers::ProviderRuntimeOptions {
                siriusclaw_dir: Some(temp.path().to_path_buf()),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from SiriClaw 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
This is an example JSON object for profile settings."#;
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "siriusclaw.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "siriusclaw.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["siriusclaw", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["siriusclaw", "restart"]);
    }
    ...
    async fn e2e_photo_attachment_rejected_by_non_vision_provider() {
        ...
        process_channel_message(
            ...
            traits::ChannelMessage {
                ...
                sender: "siriclaw_user".to_string(),
    ...
    async fn e2e_failed_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .get("test-channel_siriclaw_user")
