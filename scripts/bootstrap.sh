...
usage() {
  cat <<'USAGE'
SiriClaw installer bootstrap engine

Usage:
  ./siriclaw_install.sh [options]
  ./bootstrap.sh [options]         # compatibility entrypoint

Modes:
  Default mode installs/builds SiriClaw only (requires existing Rust toolchain).
...
Examples:
  ./siriclaw_install.sh
  ./siriclaw_install.sh --guided
  ./siriclaw_install.sh --install-system-deps --install-rust
  ./siriclaw_install.sh --prefer-prebuilt
  ./siriclaw_install.sh --prebuilt-only
  ./siriclaw_install.sh --onboard --api-key "sk-..." --provider openrouter [--model "openrouter/auto"]
  ./siriclaw_install.sh --interactive-onboard

  # Compatibility entrypoint:
  ./bootstrap.sh --docker

  # Remote one-liner
  curl -fsSL https://raw.githubusercontent.com/siriclaw-labs/siriusclaw/master/scripts/bootstrap.sh | bash

Environment:
  SIRIUSCLAW_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  SIRIUSCLAW_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  SIRIUSCLAW_DOCKER_IMAGE      Docker image tag to build/run (default: siriusclaw-bootstrap:local)
  SIRIUSCLAW_API_KEY           Used when --api-key is not provided
  SIRIUSCLAW_PROVIDER          Used when --provider is not provided (default: openrouter)
  SIRIUSCLAW_MODEL             Used when --model is not provided
  SIRIUSCLAW_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  SIRIUSCLAW_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  SIRIUSCLAW_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${SIRIUSCLAW_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${SIRIUSCLAW_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/siriclaw-labs/siriusclaw/releases/latest/download/siriusclaw-${target}.tar.gz"
  temp_dir="$(mktemp -d -t siriusclaw-prebuilt-XXXXXX)"
  archive_path="$temp_dir/siriusclaw-${target}.tar.gz"
  ...
  extracted_bin="$temp_dir/siriusclaw"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name siriusclaw -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable siriusclaw binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/siriusclaw"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/siriusclaw"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/siriusclaw-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  echo
  echo "SiriClaw guided installer"
  ...
  if prompt_yes_no "Install siriusclaw into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${SIRIUSCLAW_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set SIRIUSCLAW_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set SIRIUSCLAW_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${SIRIUSCLAW_DOCKER_IMAGE:-siriusclaw-bootstrap:local}"
  fallback_image="ghcr.io/siriclaw-labs/siriusclaw:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.siriusclaw-docker"
  else
    default_data_dir="$WORK_DIR/.siriusclaw-docker"
  fi
  docker_data_dir="${SIRIUSCLAW_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.siriusclaw" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official SiriClaw image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.siriusclaw:/siriusclaw-data/.siriusclaw"
  workspace_mount="$docker_data_dir/workspace:/siriusclaw-data/workspace"
...
Use either:
  --api-key "sk-..."
or:
  SIRIUSCLAW_API_KEY="sk-..." ./siriclaw_install.sh --docker
or run interactive:
  ./siriclaw_install.sh --docker --interactive-onboard
MSG
      exit 1
    fi
    if [[ -n "$MODEL" ]]; then
  ...
  "$CONTAINER_CLI" run --rm -it \
    ...
    -e HOME=/siriusclaw-data \
    -e SIRIUSCLAW_WORKSPACE=/siriusclaw-data/workspace \
    -v "$config_mount" \
    -v "$workspace_mount" \
    "$docker_image" \
    "${onboard_cmd[@]}"
...
REPO_URL="https://github.com/siriclaw-labs/siriusclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${SIRIUSCLAW_CONTAINER_CLI:-docker}"
API_KEY="${SIRIUSCLAW_API_KEY:-}"
PROVIDER="${SIRIUSCLAW_PROVIDER:-openrouter}"
MODEL="${SIRIUSCLAW_MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${SIRIUSCLAW_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set SIRIUSCLAW_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t siriusclaw-bootstrap-XXXXXX)"
...
Your containerized SiriClaw data is persisted under:
DONE
...
Next steps:
  ./siriclaw_install.sh --docker --interactive-onboard
  ./siriclaw_install.sh --docker --api-key "sk-..." --provider openrouter
DONE
  exit 0
fi
...
or re-run with:
  ./siriclaw_install.sh --install-rust
MSG
  exit 1
fi
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing siriusclaw to cargo bin"
...
SIRIUSCLAW_BIN=""
if have_cmd siriusclaw; then
  SIRIUSCLAW_BIN="siriusclaw"
elif [[ -x "$HOME/.cargo/bin/siriusclaw" ]]; then
  SIRIUSCLAW_BIN="$HOME/.cargo/bin/siriusclaw"
elif [[ -x "$WORK_DIR/target/release/siriusclaw" ]]; then
  SIRIUSCLAW_BIN="$WORK_DIR/target/release/siriusclaw"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$SIRIUSCLAW_BIN" ]]; then
    error "onboarding requested but siriusclaw binary is not available."
    error "Run without --skip-install, or ensure siriusclaw is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$SIRIUSCLAW_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  SIRIUSCLAW_API_KEY="sk-..." ./siriclaw_install.sh --onboard
or run interactive:
  ./siriclaw_install.sh --interactive-onboard
MSG
      exit 1
    ...
    ONBOARD_CMD=("$SIRIUSCLAW_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
