# Contributing to SiriClaw

Thanks for your interest in contributing to SiriClaw! This guide will help you get started.

## First-Time Contributors

Welcome — contributions of all sizes are valued. If this is your first contribution, here is how to get started:

1. **Find an issue.** Look for issues labeled [`good first issue`](https://github.com/siriclaw-labs/siriusclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — these are scoped for newcomers and include context to get moving quickly.
...
4. **Start with Track A.** SiriClaw uses three [collaboration tracks](#collaboration-tracks-risk-based) (A/B/C) based on risk. First-time contributors should target **Track A** (docs, tests, chore) — these require lighter review and are the fastest path to a merged PR.
...
```bash
# Clone the repo
git clone https://github.com/siriclaw-labs/siriusclaw.git
cd siriusclaw
...
SiriClaw supports layered secret management for local development and CI hygiene.
...
2. **Config file** (`~/.siriusclaw/config.toml`)
    - Persistent setup for long-term use
    - When `secrets.encrypt = true` (default), secret values are encrypted before save
    - Secret key is stored at `~/.siriusclaw/.secret_key` with restricted permissions
    - Use `sirius onboard` for guided setup
...
3. Generic env vars (`SIRIUSCLAW_API_KEY`, `API_KEY`)

Provider/model config overrides:

- `SIRIUSCLAW_PROVIDER` / `PROVIDER`
- `SIRIUSCLAW_MODEL`
...
- `~/.siriusclaw/.secret_key` or similar key files
...
- If identity-like wording is required, use SiriClaw-centric labels only (for example: `SiriClawAgent`, `SiriClawOperator`, `siriclaw_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use SiriClaw/project-native roles instead of personal or real-world identities.
...
SiriClaw's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use SiriClaw-native identifiers only (`SiriClawAgent`, `siriclaw_user`, `siriusclaw_node`).
...
- **Good identity-like label**: `SiriClawAgent`, `siriclaw_user`, `siriusclaw_node`
...
- [ ] Any required identity-like wording uses SiriClaw/project-native labels only
