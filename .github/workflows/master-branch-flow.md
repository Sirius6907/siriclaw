...
SiriClaw uses a single default branch: `master`. All contributor PRs target `master` directly. There is no `dev` or promotion branch.
...
1. External contributor opens PR from `fork/<branch>` into `siriusclaw:master`.
