...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.siriusclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.siriusclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let siriusclaw_dir = PathBuf::from(home).join(".siriusclaw");
    let auth_profiles_path = siriusclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: sirius auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: sirius auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.siriusclaw/auth-profiles.json (with expired token)
    let provider = siriusclaw::providers::create_provider("gemini", None)?;
