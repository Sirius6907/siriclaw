# Adding Boards and Tools — SiriClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to SiriClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.siriusclaw/config.toml)
sirius peripheral add nucleo-f401re /dev/ttyACM0
sirius peripheral add arduino-uno /dev/cu.usbmodem12345
sirius peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, SiriClaw can index PDF files:
...
2. **Add to config** — `sirius peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `sirius peripheral list` | List configured boards |
| `sirius peripheral add <board> <path>` | Add board (writes config) |
| `sirius peripheral flash` | Flash Arduino firmware |
| `sirius peripheral flash-nucleo` | Flash Nucleo firmware |
| `sirius hardware discover` | List USB devices |
| `sirius hardware info` | Chip info via probe-rs |
