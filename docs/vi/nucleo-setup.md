# SiriClaw trên Nucleo-F401RE — Hướng dẫn từng bước

Chạy SiriClaw trên Mac hoặc Linux. Kết nối Nucleo-F401RE qua USB. Điều khiển GPIO (LED, các pin) qua Telegram hoặc CLI.

---

## Lấy thông tin board qua Telegram (Không cần nạp firmware)

SiriClaw có thể đọc thông tin chip từ Nucleo qua USB **mà không cần nạp firmware nào**. Nhắn tin cho Telegram bot của bạn:
...
SiriClaw bao gồm mọi thứ cần thiết cho Nucleo-F401RE:

| Thành phần | Vị trí | Mục đích |
|------------|--------|---------|
| Firmware | `firmware/siriusclaw-nucleo/` | Embassy Rust — USART2 (115200), gpio_read, gpio_write |
| Serial peripheral | `src/peripherals/serial.rs` | Giao thức JSON-over-serial (giống Arduino/ESP32) |
| Flash command | `sirius peripheral flash-nucleo` | Build firmware, nạp qua probe-rs |
...
### 1.2 Nạp qua SiriClaw

Từ thư mục gốc của repo siriusclaw:

```bash
sirius peripheral flash-nucleo
```

Lệnh này build `firmware/siriusclaw-nucleo` và chạy `probe-rs run --chip STM32F401RETx`. Firmware chạy ngay sau khi nạp xong.

### 1.3 Nạp thủ công (Phương án thay thế)

```bash
cd firmware/siriusclaw-nucleo
cargo build --release --target thumbv7em-none-eabihf
probe-rs run --chip STM32F401RETx target/thumbv7em-none-eabihf/release/siriusclaw-nucleo
```
...
Thêm vào `~/.siriusclaw/config.toml`:
...
```bash
sirius agent --message "Turn on the LED on pin 13"
```
...
| 3 | `sirius peripheral flash-nucleo` |
| 4 | Thêm Nucleo vào config.toml (path = serial port của bạn) |
| 5 | `sirius daemon` hoặc `sirius agent -m "Turn on LED"` |
...
- **Lệnh GPIO bị bỏ qua** — Kiểm tra `path` trong config có khớp với serial port của bạn. Chạy `sirius peripheral list` để xác nhận.
