# SiriClaw on Nucleo-F401RE — Step-by-Step Guide

Run SiriClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

SiriClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
SiriClaw includes everything for Nucleo-F401RE:
...
| Flash command | `sirius peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via SiriClaw

From the siriusclaw repo root:

```bash
sirius peripheral flash-nucleo
```

This builds `firmware/siriusclaw-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `sirius daemon` or `sirius agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `sirius peripheral list` to verify.
