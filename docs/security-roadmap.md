# SiriClaw Security Improvement Roadmap
...
SiriClaw already has **excellent application-layer security**:
...
## Comparison: SiriClaw vs PicoClaw vs Production Grade

| Feature | PicoClaw | SiriClaw Now | SiriClaw + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`sirius audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/siriusclaw/audit.log"
sign_events = true
max_size_mb = 100
...
siriusclaw security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
siriusclaw audit --user @alice --since 24h
siriusclaw audit --risk high --violations-only
siriusclaw audit --verify-signatures

# Sandbox test
siriusclaw sandbox --test
# → Testing isolation...
...
**SiriClaw is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, SiriClaw becomes:
...
**Value**: Transforms SiriClaw from "safe for testing" to "safe for production"
