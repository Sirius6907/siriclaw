...
This guide covers native Nextcloud Talk integration for SiriClaw.
...
Add this section in `~/.siriusclaw/config.toml`:
...
sirius daemon
# or
sirius gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, SiriClaw verifies:
...
- SiriClaw ignores bot-originated webhook events (`actorType = bots`).
- SiriClaw ignores non-message/system events.
...
3. Confirm SiriClaw receives and replies in the same room.
