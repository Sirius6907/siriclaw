...
This page defines the fastest supported path to install and initialize SiriClaw.
...
Default behavior is **app-only** (build/install SiriClaw) and expects existing Rust toolchain.
...
```bash
curl -fsSL https://raw.githubusercontent.com/siriclaw-labs/siriusclaw/main/scripts/bootstrap.sh | bash
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/siriclaw-labs/siriusclaw/main/scripts/install.sh | bash
```
...
This builds a local SiriClaw image and launches onboarding inside a container while
persisting config/workspace to `./.siriusclaw-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
bootstrap auto-falls back to `podman`. You can also set `SIRIUSCLAW_CONTAINER_CLI`
explicitly (for example: `SIRIUSCLAW_CONTAINER_CLI=podman ./bootstrap.sh --docker`).
...
Docker tag (`SIRIUSCLAW_DOCKER_IMAGE`, default: `siriusclaw-bootstrap:local`); if missing,
it pulls `ghcr.io/siriclaw-labs/siriusclaw:latest` and tags it locally before running.
...
```bash
SIRIUSCLAW_API_KEY="sk-..." SIRIUSCLAW_PROVIDER="openrouter" ./bootstrap.sh --onboard
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/siriclaw-labs/siriusclaw:latest`)
- `--skip-install`
- `--provider <id>`
