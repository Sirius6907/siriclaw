...
- `cargo check` / `cargo build` appears stuck at `Checking siriusclaw` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in SiriClaw:
...
### `siriusclaw` command not found after install

Symptom:

- install succeeds but shell cannot find `siriusclaw`
...
- stop extra `sirius daemon` / `sirius channel start` processes
