# SiriClaw Config Reference (Operator-Oriented)
...
1. `SIRIUSCLAW_WORKSPACE` override (if set)
2. persisted `~/.siriusclaw/active_workspace.toml` marker (if present)
3. default `~/.siriusclaw/config.toml`

SiriClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `sirius config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `siriusclaw` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `sirius doctor traces --limit 20`
  - `sirius doctor traces --event tool_call_result --contains \"error\"`
  - `sirius doctor traces --id <trace-id>`
...
1. `SIRIUSCLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `SIRIUSCLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, SiriClaw generates one and prints an enrollment URI once.
...
| `state_file` | `~/.siriusclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `sirius estop` to engage and `sirius estop resume` to clear levels.
...
- Security-first default: SiriClaw does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `SIRIUSCLAW_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `SIRIUSCLAW_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `SIRIUSCLAW_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `SIRIUSCLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `sirius skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- SiriClaw requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
3. Validate with `sirius doctor` before restart/rollout.
...
- While `sirius channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `SIRIUSCLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `SIRIUSCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
