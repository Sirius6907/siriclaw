...
- CLI `sirius sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.siriusclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.siriusclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   siriusclaw sop list
   siriusclaw sop validate
   siriusclaw sop show deploy-prod
   ```
