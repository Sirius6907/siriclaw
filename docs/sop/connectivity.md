...
SiriClaw routes MQTT/webhook/cron/peripheral events through a unified SOP dispatcher (`dispatch_sop_event`).
...
| **Cron not firing** | daemon not running or invalid expression | run `sirius daemon`; check logs for cron parse warnings |
