# Network Deployment — SiriClaw on Raspberry Pi and Local Network

This document covers deploying SiriClaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | SiriClaw polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | SiriClaw syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — SiriClaw connects to external servers/relays. No port forwarding or public IP required.

---

## 2. SiriClaw on Raspberry Pi
...
```bash
sirius daemon --host 127.0.0.1 --port 42617
```

- Gateway binds to `127.0.0.1` — not reachable from other machines
- Telegram channel works: SiriClaw polls Telegram API (outbound)
...
```bash
sirius daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `sirius tunnel` (see tunnel docs).

3. SiriClaw will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- SiriClaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `sirius daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `sirius daemon` service).
...
- [ ] Run `sirius daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
SiriClaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `siriusclaw` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo sirius service install
```

This creates:
- Init script: `/etc/init.d/siriusclaw`
- Config directory: `/etc/siriusclaw/`
- Log directory: `/var/log/siriusclaw/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo sirius service install` automatically prepares `/etc/siriusclaw`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `siriusclaw` service user.

If no prior runtime state is available to migrate, create `/etc/siriusclaw/config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add siriusclaw default

# Start the service
sudo rc-service siriusclaw start

# Check status
sudo rc-service sirius status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service siriusclaw start` | Start the daemon |
| `sudo rc-service siriusclaw stop` | Stop the daemon |
| `sudo rc-service sirius status` | Check service status |
| `sudo rc-service siriusclaw restart` | Restart the daemon |
| `sudo sirius service status` | SiriClaw status wrapper (uses `/etc/siriusclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/siriusclaw/access.log` |
| Errors/stderr | `/var/log/siriusclaw/error.log` |

View logs:

```bash
sudo tail -f /var/log/siriusclaw/error.log
```
...
sudo rc-service siriusclaw stop
sudo rc-update del siriusclaw default

# Remove init script
sudo sirius service uninstall
```
...
- The service runs as the `siriusclaw:siriusclaw` user (least privilege)
- Config must be at `/etc/siriusclaw/config.toml` (explicit path in init script)
- If the `siriusclaw` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo sirius service install`
- [ ] Enable: `sudo rc-update add siriusclaw default`
- [ ] Start: `sudo rc-service siriusclaw start`
- [ ] Verify: `sudo rc-service sirius status`
- [ ] Check logs: `/var/log/siriusclaw/error.log`
