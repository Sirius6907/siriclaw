...
SiriClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export SIRIUSCLAW_API_KEY="your-api-key"
sirius agent
```

## llama.cpp Server (Recommended Local Setup)

SiriClaw includes a first-class local provider for `llama-server`:
...
```bash
sirius models refresh --provider llamacpp
sirius agent -m "hello"
```

You do not need to export `SIRIUSCLAW_API_KEY=dummy` for this flow.

## SGLang Server

SiriClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
sirius models refresh --provider sglang
sirius agent -m "hello"
```
...
SiriClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
sirius models refresh --provider vllm
sirius agent -m "hello"
```
