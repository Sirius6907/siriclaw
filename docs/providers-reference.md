...
3. Generic fallback env vars: `SIRIUSCLAW_API_KEY` then `API_KEY`
...
- After multimodal normalization, SiriClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, SiriClaw returns a structured capability error instead of silently ignoring images.
...
- SiriClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `sirius models refresh --provider llamacpp`
...
- Model discovery: `sirius models refresh --provider sglang`
...
- Model discovery: `sirius models refresh --provider vllm`
...
- Model discovery: `sirius models refresh --provider osaurus`
...
- Model discovery: `sirius models refresh --provider nvidia`
