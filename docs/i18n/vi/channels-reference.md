...
Tài liệu này là nguồn tham khảo chính thức về cấu hình channel trong SiriClaw.
...
6. **Trạng thái runtime cũ**: config đã thay đổi nhưng `sirius daemon` chưa được khởi động lại.

---

## 1. Namespace cấu hình

Tất cả cài đặt channel nằm trong `channels_config` trong `~/.siriusclaw/config.toml`.
...
Khi chạy `sirius channel start` (hoặc chế độ daemon), Telegram và Discord hỗ trợ chuyển đổi runtime theo phạm vi người gửi:
...
- Xem trước bộ nhớ cache model từ `sirius models refresh --provider <ID>`.
- Đây là lệnh chat runtime, không phải lệnh con CLI.

## Giao thức marker hình ảnh đầu vào

SiriClaw hỗ trợ đầu vào multimodal qua các marker nội tuyến trong tin nhắn:
...
Nếu `[channels_config.matrix]` có mặt nhưng binary được build mà không có `channel-matrix`, các lệnh `sirius channel list`, `sirius channel doctor`, và `sirius channel start` sẽ ghi log rằng Matrix bị bỏ qua có chủ ý trong bản build này.
...
access_token = "syt_..."
user_id = "@siriusclaw:matrix.example.com"   # tùy chọn, khuyến nghị cho E2EE
...
SiriClaw hỗ trợ hai backend WhatsApp:
...
5. Khởi động lại `sirius daemon` sau khi thay đổi config.
...
```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Webhook|Channel" /tmp/siriusclaw.log
```
