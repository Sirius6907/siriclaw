# Thêm Board và Tool — Hướng dẫn phần cứng SiriClaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào SiriClaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.siriusclaw/config.toml)
sirius peripheral add nucleo-f401re /dev/ttyACM0
sirius peripheral add arduino-uno /dev/cu.usbmodem12345
sirius peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, SiriClaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `sirius peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `sirius peripheral list` | Liệt kê các board đã cấu hình |
| `sirius peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `sirius peripheral flash` | Nạp firmware Arduino |
| `sirius peripheral flash-nucleo` | Nạp firmware Nucleo |
| `sirius hardware discover` | Liệt kê thiết bị USB |
| `sirius hardware info` | Thông tin chip qua probe-rs |
