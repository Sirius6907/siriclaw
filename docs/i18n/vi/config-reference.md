...
1. Biến `SIRIUSCLAW_WORKSPACE` (nếu được đặt)
2. Marker `~/.siriusclaw/active_workspace.toml` (nếu có)
3. Mặc định `~/.siriusclaw/config.toml`

SiriClaw ghi log đường dẫn config đã giải quyết khi khởi động ở mức `INFO`:

- `Config loaded` với các trường: `path`, `workspace`, `source`, `initialized`

Lệnh xuất schema:

- `sirius config schema` (xuất JSON Schema draft 2020-12 ra stdout)
...
| `otel_service_name` | `siriusclaw` | Tên dịch vụ gửi đến OTLP collector |
...
1. `SIRIUSCLAW_PROVIDER` (ghi đè tường minh, luôn thắng khi có giá trị)
...
- Dùng `SIRIUSCLAW_PROVIDER` khi cố ý muốn biến môi trường ghi đè provider đã cấu hình.
...
- Mặc định an toàn: SiriClaw **không** clone hay đồng bộ `open-skills` trừ khi `open_skills_enabled = true`.
- Ghi đè qua biến môi trường:
  - `SIRIUSCLAW_OPEN_SKILLS_ENABLED` chấp nhận `1/0`, `true/false`, `yes/no`, `on/off`.
  - `SIRIUSCLAW_OPEN_SKILLS_DIR` ghi đè đường dẫn kho khi có giá trị.
- Thứ tự ưu tiên: `SIRIUSCLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` trong `config.toml` → mặc định `false`.
...
- SiriClaw yêu cầu Composio v3 tools với `toolkit_versions=latest` và thực thi với `version="latest"` để tránh bản tool mặc định cũ.
...
3. Kiểm tra bằng `sirius doctor` trước khi khởi động lại/triển khai.
...
- Khi `sirius channel start` đang chạy, thay đổi `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url` và `reliability.*` được áp dụng nóng từ `config.toml` ở tin nhắn tiếp theo.
