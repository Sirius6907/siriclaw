...
|---|---|---|
| Foreground runtime | `sirius daemon` | gỡ lỗi cục bộ, phiên ngắn |
| Foreground gateway only | `sirius gateway` | kiểm thử webhook endpoint |
| User service | `sirius service install && sirius service start` | runtime được quản lý liên tục bởi operator |
...
|---|---|---|
| Tính hợp lệ của config | `sirius doctor` | không có lỗi nghiêm trọng |
| Kết nối channel | `sirius channel doctor` | các channel đã cấu hình đều khoẻ mạnh |
| Tóm tắt runtime | `sirius status` | provider/model/channels như mong đợi |
| Heartbeat/trạng thái daemon | `~/.siriusclaw/daemon_state.json` | file được cập nhật định kỳ |
...
1. Nếu các channel vẫn thất bại, kiểm tra allowlist và thông tin xác thực trong `~/.siriusclaw/config.toml`.
...
1. sao lưu `~/.siriusclaw/config.toml`
2. chỉ áp dụng một thay đổi logic tại một thời điểm
3. chạy `sirius doctor`
