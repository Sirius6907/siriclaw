...
[INFO] ✓ Audit logging enabled (~/.config/siriusclaw/audit.log)
...
$ siriusclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/siriusclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ sirius config set security.sandbox.enabled false

# Bật backend cụ thể
$ sirius config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ sirius config set security.resources.max_memory_mb 2048
```
...
```bash
$ sirius onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `sirius security --status`
