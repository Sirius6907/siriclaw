# Triển khai mạng — SiriClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai SiriClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | SiriClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | SiriClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — SiriClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. SiriClaw trên Raspberry Pi
...
Chỉnh sửa `~/.siriusclaw/config.toml`:
...
- Channel Telegram hoạt động: SiriClaw poll Telegram API (outbound)
...
```bash
sirius daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `sirius tunnel` (xem tài liệu tunnel).

3. SiriClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- SiriClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `sirius daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `sirius daemon`).
...
- [ ] Chạy `sirius daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
