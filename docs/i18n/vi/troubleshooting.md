...
- `cargo check` / `cargo build` dừng lâu ở `Checking siriusclaw`
...
### Không tìm thấy lệnh `siriusclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `siriusclaw`
...
Xác minh `~/.siriusclaw/config.toml`:
...
- Dừng các tiến trình `sirius daemon` / `sirius channel start` thừa
