...
This document is the canonical reference for channel configuration in SiriClaw.
...
6. **Stale runtime state**: config changed but `sirius daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.siriusclaw/config.toml`.
...
When running `sirius channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `sirius models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

SiriClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `sirius channel list`, `sirius channel doctor`, and `sirius channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@siriusclaw:matrix.example.com"   # optional, recommended for E2EE
...
SiriClaw supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.siriusclaw/state/whatsapp-web/session.db"
...
port = 6697
nickname = "siriusclaw-bot"
username = "siriusclaw"              # optional
channels = ["#siriusclaw"]
allowed_users = ["*"]
...
- `SIRIUSCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `SIRIUSCLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `sirius daemon` after config changes.
...
```bash
RUST_LOG=info sirius daemon 2>&1 | tee /tmp/siriusclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/siriusclaw.log
```
