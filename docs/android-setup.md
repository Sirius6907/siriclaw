...
SiriClaw provides prebuilt binaries for Android devices.
...
The easiest way to run SiriClaw on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/siriclaw-labs/siriusclaw/releases/latest/download/siriusclaw-aarch64-linux-android.tar.gz
tar xzf siriusclaw-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/siriclaw-labs/siriusclaw/releases/latest/download/siriusclaw-armv7-linux-androideabi.tar.gz
tar xzf siriusclaw-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run SiriClaw outside Termux:

```bash
# From your computer with ADB
adb push siriusclaw /data/local/tmp/
adb shell chmod +x /data/local/tmp/siriusclaw
adb shell /data/local/tmp/siriusclaw --version
```
