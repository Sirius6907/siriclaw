#!/bin/bash
# SiriClaw Telegram Integration Test Suite
...
if [ -f "target/release/siriusclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/siriusclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/siriusclaw 2>/dev/null || stat -c%s target/release/siriusclaw)
...
CONFIG_PATH="$HOME/.siriusclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'sirius onboard' first"
    fi
else
    warn "No config file found - run 'sirius onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/sirius channel doctor 2>&1 || true)
END_TIME=$(date +%s)
