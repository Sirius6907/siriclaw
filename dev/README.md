# SiriClaw Development Environment

A fully containerized development sandbox for SiriClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`siriusclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `siriusclaw` CLI commands manually, debug the binary, or check logs internally.

- **Path**: `/siriusclaw-data`
- **User**: `nobody` (65534)
...
- **Agent**: `/siriusclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.siriusclaw` (mounted to `/siriusclaw-data/.siriusclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.siriusclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `siriusclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=siriusclaw-target`) to speed repeat local image builds.
