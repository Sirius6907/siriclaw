...
function ensure_config {
    CONFIG_DIR="$HOST_TARGET_DIR/.siriusclaw"
    CONFIG_FILE="$CONFIG_DIR/config.toml"
    WORKSPACE_DIR="$CONFIG_DIR/workspace"

    if [ ! -f "$CONFIG_FILE" ]; then
        echo -e "${YELLOW}⚙️  Config file missing in target/.siriusclaw. Creating default dev config from template...${NC}"
...
function print_help {
    echo -e "${YELLOW}SiriClaw Development Environment Manager${NC}"
    ...
    echo -e "  ${GREEN}agent${NC}   Enter Agent (SiriClaw CLI)"
...
case "$1" in
    up)
        ...
        echo -e "   - Config: target/.siriusclaw/config.toml (Edit locally to apply changes)"
        ;;
    ...
    shell)
        ...
        docker exec -it siriusclaw-sandbox /bin/bash
        ;;

    agent)
        echo -e "${GREEN}🤖 Entering Agent Container (SiriClaw)... (Type 'exit' to leave)${NC}"
        docker exec -it siriusclaw-dev /bin/bash
        ;;
    ...
    clean)
        echo -e "${RED}⚠️  WARNING: This will delete 'target/.siriusclaw' data and Docker volumes.${NC}"
        read -p "Are you sure? (y/N) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            docker compose -f "$COMPOSE_FILE" down -v
            rm -rf "$HOST_TARGET_DIR/.siriusclaw"
