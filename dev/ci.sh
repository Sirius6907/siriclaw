...
build_smoke_image() {
  if docker buildx version >/dev/null 2>&1; then
    ...
    local build_args=(
      ...
      -t siriusclaw-local-smoke:latest
      .
    )
    ...
    DOCKER_BUILDKIT=1 docker build --target dev -t siriusclaw-local-smoke:latest .
  fi
}

print_help() {
  cat <<'EOF'
SiriClaw Local CI in Docker
...
case "$1" in
  ...
  docker-smoke)
    build_smoke_image
    docker run --rm siriusclaw-local-smoke:latest --version
    ;;
