//! Performance benchmarks for SiriClaw hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/siriclaw-labs/siriusclaw/issues/618 (item 7)

use criterion::{criterion_group, criterion_main, Criterion};
use std::hint::black_box;
use std::sync::{Arc, Mutex};

use siriusclaw::agent::agent::Agent;
use siriusclaw::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use siriusclaw::config::MemoryConfig;
use siriusclaw::memory;
use siriusclaw::memory::{Memory, MemoryCategory};
use siriusclaw::observability::{NoopObserver, Observer};
use siriusclaw::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
use siriusclaw::tools::{Tool, ToolResult};
...
<tool_call>
{"name": "search", "arguments": {"query": "siriusclaw architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "siriusclaw"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about sirius agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("sirius agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
